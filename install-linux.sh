...
if [ ${#MISSING_DEPS[@]} -ne 0 ]; then
    ...
    echo "  Ubuntu/Debian: sudo apt-get install curl tar sed"
    echo "  CentOS/RHEL:   sudo yum install curl tar sed"
    echo "  Arch Linux:    sudo pacman -S curl tar sed"
    exit 1
fi
...
get_target_triple() {
    local arch=$1
    # Check if musl should be used (set CHRONOVA_CLI_MUSL=1 for musl builds)
...
WAKATIME_CLI_NAME="wakatime-cli-linux-${ARCH}"
...
if [ -z "$API_KEY" ] && [ -t 0 ]; then
    ...
    echo -e "(${YELLOW}Click the link above or copy-paste it into your browser${NC})"
    echo ""
