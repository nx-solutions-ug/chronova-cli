...
impl ChronovaSyncManager {
    ...
    pub async fn start_background_sync(&self) -> Result<(), SyncError> {
        ...
        tokio::spawn(async move {
            tracing::info!(
                "Starting background sync with interval: {} seconds",
    ...
    pub async fn sync_with_retry(
        &self,
        operation: impl Fn() -> Result<SyncResult, SyncError> + Send + Sync,
        ...
        while attempt <= self.retry_strategy.max_attempts {
            ...
            match operation() {
                ...
                Err(error) => {
                    ...
                    tracing::info!(
                        "Sync operation failed (attempt {}), retrying in {} seconds: {}",
                        attempt,
                        delay.as_secs(),
                        error
                    );
    ...
    pub fn record_sync_metrics(&self, result: &SyncResult) {
        ...
        if result.error.is_none() && result.failed_count == 0 {
            self.successful_sync_operations
        ...
        *last_size_guard = Some(queue_size);
        ...
        tracing::debug!(
            ...
            queue_utilization_percent =
                (queue_size as f64 / self.config.max_queue_size as f64) * 100.0,
            "Queue size updated"
        );
    ...
    pub fn calculate_latency_metrics(
        &self,
        start_time: Instant,
        end_time: Instant,
        count: usize,
...
impl SyncManager for ChronovaSyncManager {
    async fn sync_pending(&self) -> Result<SyncResult, SyncError> {
        ...
        loop {
            ...
            let avg_latency =
                self.calculate_latency_metrics(batch_start, batch_end, pending_res.len());
...
mod tests {
    ...
    fn test_sync_status_from_str() {
        ...
        assert_eq!(
            SyncStatus::from("permanent_failure"),
            SyncStatus::PermanentFailure
        );
    ...
    fn test_sync_status_to_string() {
        ...
        assert_eq!(
            String::from(SyncStatus::PermanentFailure),
            "permanent_failure"
        );
    }
    ...
    fn test_sync_error_variants() {
        ...
        assert!(config_error
            .to_string()
            .contains("Invalid configuration: missing api key"));

        // Test Unknown error
        let unknown_error = SyncError::Unknown("something went wrong".to_string());
        assert!(unknown_error
            .to_string()
            .contains("Unknown error: something went wrong"));
    }
    ...
    async fn test_connectivity_check_failure() {
        ...
        assert!(
            result.is_ok(),
            "Connectivity check should return Ok even on failure"
        );
    ...
    async fn test_connectivity_caching() {
        ...
        assert!(
            sync_manager.get_cached_connectivity(),
            "Cached state should be true"
        );
    }
    ...
    async fn test_connectivity_monitoring_start() {
        ...
        assert!(
            sync_manager.get_cached_connectivity(),
            "Monitoring should update cache"
        );
        ...
        assert!(
            time_since_check.unwrap() < std::time::Duration::from_secs(1),
    ...
    fn test_is_retryable_error() {
        ...
        assert!(RetryStrategy::is_retryable_error(&SyncError::Network(
            "test".to_string()
        )));
        assert!(RetryStrategy::is_retryable_error(&SyncError::RateLimit(
            "test".to_string()
        )));
        assert!(RetryStrategy::is_retryable_error(&SyncError::Database(
            "test".to_string()
        )));
        assert!(RetryStrategy::is_retryable_error(
            &SyncError::Serialization("test".to_string())
        ));
        assert!(RetryStrategy::is_retryable_error(&SyncError::Unknown(
            "test".to_string()
        )));

        // Test non-retryable errors
        assert!(!RetryStrategy::is_retryable_error(&SyncError::Auth(
            "test".to_string()
        )));
        assert!(!RetryStrategy::is_retryable_error(&SyncError::Config(
            "test".to_string()
        )));
    }
    ...
    async fn test_start_all_services() {
        ...
        assert!(
            sync_manager.get_cached_connectivity(),
            "Connectivity monitoring should update cache"
        );
    }
