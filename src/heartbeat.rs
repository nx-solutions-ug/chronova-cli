...
impl HeartbeatManager {
    ...
    /// qualify, but an owned, freshly-opened `Queue` does. Tests call this
    /// directly against a manager's own (isolated) queue, so it's the same
    /// function under test in both places, not a production/test split.
