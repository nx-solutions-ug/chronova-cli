...
async fn test_observability_integration() {
    ...
    assert_eq!(
        sync_manager.total_sync_operations.load(Ordering::Relaxed),
        0
    );
    assert_eq!(
        sync_manager
            .successful_sync_operations
            .load(Ordering::Relaxed),
        0
    );
    assert_eq!(
        sync_manager.failed_sync_operations.load(Ordering::Relaxed),
        0
    );
    assert_eq!(
        sync_manager.total_sync_latency_ms.load(Ordering::Relaxed),
        0
    );
