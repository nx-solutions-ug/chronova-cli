...
async fn test_retry_mechanism_integration() {
    ...
    let failed_heartbeats = queue.get_pending(Some(1000), Some(SyncStatus::Failed)).unwrap();
    ...
    let permanent_failures = queue.get_pending(None, Some(SyncStatus::PermanentFailure)).unwrap();
